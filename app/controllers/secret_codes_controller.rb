class SecretCodesController < ApplicationController
  ...
  def new
    @secret_code = SecretCode.new(bulk_generate: 10)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @secret_code.valid?
        (@secret_code.bulk_generate||0).times do 
          @secret_code = SecretCode.create
        end
        format.html { redirect_to secret_codes_url, notice: 'Secret code was successfully created.' }
  ...
  private
    ...
    def secret_code_params
      params.require(:secret_code).permit(:code, :bulk_generate)
    end
end
